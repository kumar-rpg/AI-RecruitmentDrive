...
const S = StyleSheet.create({
  page: { fontFamily: 'Helvetica', fontSize: 8, paddingTop: 28, paddingBottom: 28, paddingLeft: 28, paddingRight: 28, color: '#000' },
  // Header
  headerBox: { borderWidth: 1, borderColor: '#000', marginBottom: 8, paddingTop: 6, paddingBottom: 6, paddingLeft: 8, paddingRight: 8 },
  headerCo: { fontFamily: 'Helvetica-Bold', fontSize: 14 },
  headerForm: { fontFamily: 'Helvetica-Bold', fontSize: 10, marginTop: 3 },
  headerMeta: { fontSize: 7.5, marginTop: 3 },
  // Section title bar
  sectionBar: { fontFamily: 'Helvetica-Bold', fontSize: 8.5, backgroundColor: '#d8d8d8', paddingTop: 3, paddingBottom: 3, paddingLeft: 5, paddingRight: 5, marginBottom: 0 },
  // Table helpers
  row: { flexDirection: 'row' },
  // Base cell — call cellStyle() helper below instead of using directly
  cell: { borderWidth: 1, borderColor: '#000', paddingTop: 2, paddingBottom: 3, paddingLeft: 4, paddingRight: 4, flex: 1 },
  noLeft: { borderLeftWidth: 0 },
  noTop: { borderTopWidth: 0 },
  lbl: { fontFamily: 'Helvetica-Bold', fontSize: 6.5, marginBottom: 1 },
  val: { fontSize: 8, minHeight: 9 },
  // Checkbox
  cbRow: { flexDirection: 'row', flexWrap: 'wrap', marginTop: 2 },
  cbItem: { flexDirection: 'row', alignItems: 'center', marginRight: 10, marginBottom: 2 },
  cbBox: { width: 7, height: 7, borderWidth: 1, borderColor: '#000', marginRight: 2 },
  cbBoxOn: { width: 7, height: 7, borderWidth: 1, borderColor: '#000', marginRight: 2, backgroundColor: '#000' },
  // Section spacing
  section: { marginBottom: 6 },
  rowSub: { fontFamily: 'Helvetica-Bold', fontSize: 7.5, marginBottom: 3, marginTop: 5 },
});
...
// Plain value cell (no label — for table body rows)
function TCell({ value, flex, noLeft, noTop }) {
  const style = [S.cell];
  if (noLeft) style.push(S.noLeft);
  if (noTop) style.push(S.noTop);
  if (flex) style.push({ flex });
  return <View style={style}><Text style={S.val}>{v(value)}</Text></View>;
}
...
function TRow({ cols, data, noTop }) {
  return (
    <View style={S.row}>
      {cols.map((c, i) => (
        <TCell key={i} value={data[c.key]} flex={c.flex || 1} noLeft={i > 0} noTop={noTop} />
      ))}
    </View>
  );
}
...
function CheckItem({ label, checked }) {
  return (
    <View style={S.cbItem}>
      <View style={checked ? S.cbBoxOn : S.cbBox} />
      <Text>{label}</Text>
    </View>
  );
}
...
function SectionTitle({ children }) {
  return <Text style={[S.sectionBar, { marginBottom: 0 }]}>{children}</Text>;
}
...
function PersonalSection({ d }) {
  ...
  return (
    <View style={S.section}>
      ...
      <View style={[S.cell, S.noTop]}>
        <Text style={S.lbl}>Driving License(s)</Text>
        <View style={S.cbRow}>
      ...
      {langs.map((l, i) => {
        ...
        return (
          <View key={i} style={S.row}>
            <TCell value={l.label} flex={1.5} noTop />
            <TCell value={data.speaking} noLeft noTop />
            <TCell value={data.reading} noLeft noTop />
            <TCell value={data.writing} noLeft noTop />
          </View>
        );
      })}
    </View>
...
function JobBlock({ label, emp }) {
  ...
  return (
    <View style={{ marginBottom: 4 }}>
      ...
      <View style={[S.cell, S.noTop]}>
        <Text style={S.lbl}>Reason for Leaving</Text>
...
function EmploymentSection({ d }) {
  ...
  const hasAny = d.current_employment && Object.values(d.current_employment).some(Boolean);
  const hasPrev = prev.some((p) => Object.values(p).some(Boolean));
  if (!hasAny && !hasPrev) return null;
  return (
    <View style={S.section}>
...
function GeneralSection({ d }) {
  ...
  return (
    <View style={S.section}>
      ...
      <View style={S.row}>
        <Cell label="Expected Salary (RM)" value={d.expected_salary} />
      ...
      <View style={[S.cell, S.noTop]}>
        <Text style={S.lbl}>How did you hear about this vacancy?</Text>
        <View style={S.cbRow}>
      ...
      <View style={[S.cell, S.noTop]}>
        <Text style={S.lbl}>Have you ever been convicted of any criminal offence?</Text>
        <Text style={S.val}>
      ...
      <View style={[S.cell, S.noTop]}>
        <Text style={S.lbl}>Do you have any health condition that may affect your work performance?</Text>
        <Text style={S.val}>
      ...
      {d.is_pregnant !== null && d.is_pregnant !== undefined && (
        <View style={[S.cell, S.noTop]}>
          <Text style={S.lbl}>Currently Pregnant?</Text>
          <Text style={S.val}>
      ...
      {hasCortex && (
        <View style={[S.cell, S.noTop]}>
          <Text style={S.lbl}>Known connections at Cortex Robotics</Text>
          {cortex.filter((c) => Object.values(c).some(Boolean)).map((c, i) => (
            <Text key={i} style={S.val}>
    ...
    <View style={S.section}>
      ...
      <View style={S.cell}>
        <Text style={[S.val, { lineHeight: 1.6, marginBottom: 8 }]}>
          I declare that the information given in this application is true and correct to the best of my
          knowledge and belief. I understand that any false statement or omission of facts on my part will
          be sufficient reason for rejection of my application or for dismissal if I have been employed.
        </Text>
        <View style={S.row}>
        ...
        <View style={[S.cell, S.noTop, { height: 28 }]}>
