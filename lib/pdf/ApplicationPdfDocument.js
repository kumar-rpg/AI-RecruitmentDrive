...
const S = StyleSheet.create({
  ...
  // Cell inside a row — flex set explicitly per cell, never in base style
  cell: {
...
// flex is always set explicitly — never inherited from base style
function Cell({ label, value, flex = 1, noLeft, noTop }) {
  const style = [S.cell, { flex }];
...
function THead({ cols }) {
  return (
    <View style={S.row}>
      {cols.map((c, i) => (
        <View key={i} style={[S.cell, { flex: c.flex || 1 }, i > 0 ? S.noLeft : null]}>
  ...
  return (
    <View style={S.section}>
      <SectionStart title="A.  PERSONAL PARTICULARS">
        {/* Rows 1-3 on the left, photo box on the right spanning all three */}
        <View style={{ flexDirection: 'row' }}>
          ...
          {/* Photo box — no left border; right borders of adjacent cells act as divider */}
          {/* 3 rows × ~21pt each = ~63pt; no padding so image fills edge-to-edge */}
          <View style={[S.cell, { width: 70, borderLeftWidth: 0, paddingTop: 0, paddingBottom: 0, paddingLeft: 0, paddingRight: 0, alignItems: 'center', justifyContent: 'center' }]}>
            {d.photo_url
              ? <Image src={d.photo_url} style={{ width: 70, height: 63, objectFit: 'cover', objectPosition: 'center top' }} />
              : <Text style={{ fontSize: 6, color: '#bbb', textAlign: 'center' }}>Photo not{'\n'}provided</Text>
            }
          </View>
        </View>
      </SectionStart>
    ...
    <View style={S.section}>
      <SectionStart title="D.  LANGUAGE PROFICIENCY  (0 = None, 10 = Excellent)">
        <View style={S.row}>
          ...
          <View style={[S.cell, { flex: 1 }, S.noLeft]}><Text style={S.lbl}>Speaking</Text></View>
          <View style={[S.cell, { flex: 1 }, S.noLeft]}><Text style={S.lbl}>Reading</Text></View>
          <View style={[S.cell, { flex: 1 }, S.noLeft]}><Text style={S.lbl}>Writing</Text></View>
        </View>
      </SectionStart>
      {langs.map((l, i) => {
        ...
        return (
          <View key={i} style={S.row}>
            ...
            <Cell value={data.speaking} flex={1} noLeft noTop />
            <Cell value={data.reading}  flex={1} noLeft noTop />
            <Cell value={data.writing}  flex={1} noLeft noTop />
          </View>
        );
      })}
    </View>
