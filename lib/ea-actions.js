...
export async function getOrCreateDraft() {
  ...
  // ignoreDuplicates makes concurrent first-load renders safe: whichever runs
  // second is a no-op, and both read back the same prefilled row.
  const prefill = {
  ...
  await supabaseAdmin()
    .from('employment_applications')
    .upsert(prefill, { onConflict: 'auth_user_id', ignoreDuplicates: true });
...
function nullifyEmpty(obj) {
  ...
  if (obj !== null && typeof obj === 'object') {
    return Object.fromEntries(
      Object.entries(obj).map(([k, v]) => [k, nullifyEmpty(v)])
    );
  }
  return obj === '' ? null : obj;
}
...
export async function saveDraft(section) {
  ...
  if (user.app_metadata?.role === 'admin') {
    return { error: 'This browser is signed in as HR admin. Sign out and sign back in as the applicant to save.' };
  }

  const { error } = await supabase
    .from('employment_applications')
    .upsert(
      { auth_user_id: user.id, ...nullifyEmpty(section), last_saved_at: new Date().toISOString() },
...
export async function submitApplication() {
  ...
  if (user.app_metadata?.role === 'admin') {
    return { error: 'This browser is signed in as HR admin. Sign out and sign back in as the applicant to submit.' };
  }
...
export async function getInterviewApplicants() {
  ...
  // Auth accounts are keyed by lowercase email (Supabase normalises it)
  const { data: authData } = await supabaseAdmin().auth.admin.listUsers({ perPage: 1000 });
  const authMap = {};
  (authData?.users || []).forEach((u) => { authMap[u.email?.toLowerCase()] = u.id; });

  const rows = (applicants.data || []).map((a) => ({
    ...a,
    authUserId: authMap[a.email?.trim().toLowerCase()] ?? null,
  }));

  // Match forms by the applicant's own login, not by the email typed on the form
  const authIds = rows.map((r) => r.authUserId).filter(Boolean);
  const { data: apps } = authIds.length
    ? await supabaseAdmin()
        .from('employment_applications')
        .select('auth_user_id, status, submitted_at')
        .in('auth_user_id', authIds)
    : { data: [] };

  const appMap = {};
  (apps || []).forEach((a) => { appMap[a.auth_user_id] = a; });

  return {
    applicants: rows.map((r) => ({
      ...r,
      formStatus: appMap[r.authUserId]?.status ?? null,
      submittedAt: appMap[r.authUserId]?.submitted_at ?? null,
    })),
  };
}
