...
export async function getOrCreateDraft() {
  ...
  if (existing) {
    // Patch position_applied if it's missing (e.g. draft pre-dates this field)
    if (!existing.position_applied && applicant?.position) {
      await supabase
        .from('employment_applications')
        .update({ position_applied: applicant.position })
        .eq('auth_user_id', user.id);
      return { ...existing, position_applied: applicant.position };
    }
    return existing;
  }
