...
/**
 * Calendar-accurate duration between two yyyy-mm-dd dates, as whole months
 * plus a remainder of days — e.g. 2026-09-01 -> 2026-12-01 is exactly
 * { months: 3, days: 0 }, matching how "3 months" reads to a person, rather
 * than a day-count divided by 30.44. Returns null for missing/invalid/
 * inverted input.
 */
...
/** "3 months", "5 months, 26 days", "28 days" — for display next to the date pickers. */
export function formatDuration(breakdown) {
  if (!breakdown) return '—';
  const { months, days } = breakdown;
  const parts = [];
